...
import { Button, Divider, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, useDisclosure, Stack } from '@chakra-ui/react'
...
function Mail() {
    ...
    const handleChange = (event) => {
        ...
        if (name == 'category') {
            setFilter((old) => {
                return {
                    ...old,
                    category: old.category.includes(value) ? old.category : [...old.category, value]
                }
            })
        } else {
            setFilter((old) => {
                return {
                    ...old,
                    individual: old.individual.includes(value) ? old.individual : [...old.category, value]
                }
            })
        }
    ...
    const handleSelect = (item) => {
        setFilter((old) => {
            return {
                ...old,
                individual: old.individual.includes(item?._id) ? old.individual : [...old.individual, item?._id]
            }
        })
        setsearchResult([])
    }
    ...
    useEffect(() => {
        if (name) {
            ...
            timeout = setTimeout(() => {
                ApiPost('/admin/member/get/all', { search: name, limit: 10, page: 1 }).then((response) => {
        ...
        <>
            <Container minW={'100%'} mt={16} bg={'white'}>
                <Box textAlign={'right'} py='4'>
                    <Button onClick={onOpen} > <AiOutlinePlus /> &nbsp; Select  Members  </Button>
                </Box>
                ...
                <Stack direction='row' my={2} textAlign="right" spacing={4} align='center' justifyContent="right">
                ...
                <Heading textAlign={'center'} mt={4}>Mail Preview</Heading>

                <Box height={600} overflowY={'scroll'} bg={'#f1f1f1a3'} p={4}>     <div
            ...
            <Modal finalFocusRef={finalRef} isOpen={isOpen} onClose={onClose}>
                ...
                <ModalContent>
                    ...
                    <ModalBody>
                        ...
                        <Box py={4}>
                            {
                                filter?.category?.map((cat) => <Tags value={categories_map[cat]} remove={() => handleTagRemove(cat)} />)
                            }
                        </Box>

                        <Input type='text' mt={4} placeholder='Search Individual' name='name' value={name} onChange={(event) => setName(event.target.value)} />

                        <Resultmenu itemArray={searchResult} choose={handleSelect} />

                        <Box py={4}>
                            {
                                filter?.individual?.map((ind) => <Tags value={(individual_map[ind] || ind)} remove={() => handleTagRemoveForIndividual(ind)} />)
                            }
                        </Box>
                    </ModalBody>
