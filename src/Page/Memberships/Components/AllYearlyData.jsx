...
const AllYearlyData = ({ details }) => {
    ...
    useEffect(() => {
        ...
        while (tempArr.length) newArr.push(tempArr.splice(0, 7));
        setData(newArr);
    }, [details]);
    return (
        <div id='card_datas'>
            <div style={{ position: 'absolute', left: '-9999px', bottom: 0 }}>
                {datas.map((post, i) => (
                    <div key={post._id} id={`datas-${post[0]._id}`} className="card-container px-3 pt-3">
            ...
            <div>
                <Button onClick={downloadAllIdcardsPDF} disabled={isGeneratingPDF}>
                    {isGeneratingPDF ? 'Generating PDF...' : 'Directory without Photo(member)'}
                </Button>
            </div>
        </div>
    );
